...
if [[ ! -f "${cov_file}" ]]; then
    ...
    Rscript /rsrch5/home/epi/sthead/isoqtl_lr_breast/scripts/TCGA/child_HCP_by_param.R ${assembly} ${do_ods} ${lstpm} ${do_vst} ${do_tmm} ${do_bc} ${k} ${method} ${gen_pcs_file} ${num_gen_pcs}
else
...
if [[ ! -f "${bed_file}.gz" ]]; then
    echo "bgzipping and tabixing tumor bed file..."
    bgzip ${bed_file} && tabix -p bed ${bed_file}.gz
else
    echo "bed file already bgzipped and tabixed..."
fi
