#!/usr/bin/env Rscript

########################################################################
# change library to local
########################################################################
myPaths <- .libPaths()
myPaths <- c("/rsrch5/home/epi/sthead/R/x86_64-pc-linux-gnu-library/4.3",myPaths)
.libPaths(myPaths)

########################################################################
# load dependencies
########################################################################
library(data.table)
library(dplyr)

# clear out environment
rm(list = ls())

####################################################################################
# parse arguments
####################################################################################
args <- commandArgs(trailingOnly = TRUE)
assembly <- as.character(args[1]) # gencodev45 or veigaTum
do_ods <- as.numeric(args[2]) # 0 or 1
lstpm <- as.numeric(args[3]) # 0 or 1
do_vst <- as.numeric(args[4]) # 0 or 1
do_tmm <- as.numeric(args[5]) # 0 or 1
do_bc <- as.numeric(args[6])# 0 or 1
k <- as.numeric(args[7])
method <- as.character(args[8]) # "HC" or "PC"
gen_pcs_file <- as.character(args[9])
num_gen_pcs <- as.numeric(args[10])

# assembly="gencodev45"
# do_ods=1
# lstpm=0
# do_vst=1
# do_tmm=1
# do_bc=0
# k=15
# method="HC"
# gen_pcs_file="/rsrch5/home/epi/bhattacharya_lab/data/TCGA/BRCA/AffySNP6/VCF/normal/imputed/MAF0.01_passQC/autosome_unrelated.eigenvec"
# num_gen_pcs=5

########################################################################
# helper functions
########################################################################
hidden_convariate_linear <- function(F,Y,k,lambda,lambda2,lambda3,iter) {
  ## Use Example
  # hcp = hidden_convariate_linear(standardize(datSeq), standardize(t(datExpr)),k=10,iter = 100)
  #
  # function [Z,B,U,o,error,error1,error2,dz,db,du] = hidden_covariate_linear(F,Y,k,lambda,lambda2,lambda3,iter);
  # input:
  #      F: a matrix nxd of known covariates, where n is the number of
  #      subjects and d is the number of known covariates. *must be standardize (columns have 0 mean and constant SS).
  #      Y: a matrix of nxg of expression data (must be standardized (columns
  #      scaled to have constant SS and mean 0). ** use standardize function to standardize F and Y.
  #      k: number of inferred hidden components (k is an integer)
  #      lambda, lambda2, lambda3 are model parameters
  #      (optional) iter: number of iterations (default = 100);
  #
  #      note: k>0, lambda>0, lambda2>0, lambda3>0 must be set by the user based on the data at hand. one can set these values
  #      using cross-validation, by evaluating the "performance" of the  resulting residual data on a desired task.
  #      typically, if lambda>5, then hidden factors match the known covariates closely.
  #
  # objective:
  #
  # this function solves the following problem:
  # argmin_{Z,B,U}   ||Y-Z*B||_2 + \lambda*||Z-F*U||_2 + \lambda2*||B||_2 + \lambda_3||U||_2
  #
  # output:
  #      Z: matrix of hidden components, dimensionality: nxk
  #      B: matrix of effects of hidden components, dimensionality: kxg
  #      o: value of objective function on consecutive iterations.
  #
  # to use the residual data: Residual = Y - Z*B
  library(MASS)
  library(pracma)
  
  tol = 1e-6;
  
  U = matrix(0, nrow=dim(F)[2],k)
  Z = matrix(0, nrow=dim(F)[1],k)
  B = matrix(runif(dim(Z)[2]*dim(Y)[2]), nrow=dim(Z)[2], ncol=dim(Y)[2])
  F = as.matrix(F)
  
  n1 = dim(F)[1]
  d1 = dim(F)[2]
  
  n2 = dim(Y)[1]
  d2 = dim(Y)[2]
  
  if(n1!=n2)    stop("number of rows in F and Y must agree")
  
  if (k<1 | lambda<1e-6 | lambda2<1e-6 | lambda3<1e-6 ) {
    stop("lambda, lambda2, lambda3 must be positive and/or k must be an integer");
  }
  
  o = vector(length=iter)
  
  for (ii in 1:iter) {
    print(ii)
    o[ii] = sum((Y - Z%*%B)^2) + sum((Z -  F%*%U)^2)*lambda + 
      (sum(B^2))*lambda2 + lambda3*(sum(U^2));
    Z = (Y %*% t(B) + lambda * F %*%U) %*% ginv(B %*% t(B) + lambda * diag(dim(B)[1]))
    B = mldivide(t(Z) %*% Z + lambda2 * diag(dim(Z)[2]), (t(Z) %*% Y))
    U = mldivide(t(F) %*% F * lambda + lambda3 * diag(dim(U)[1]), lambda * t(F) %*% Z)
    
    if(ii > 1 &&  (abs(o[ii]-o[ii-1])/o[ii]) < tol)  break
  }
  
  error =  sum((Y - Z%*%B)^2) / sum(Y^2)  + sum((Z - F%*%U)^2)/sum((F%*%U)^2)
  error1 = sum((Y - Z%*%B)^2) / sum(Y^2);
  error2 = sum((Z - F%*%U)^2) / sum((F%*%U)^2);
  
  dz = Z%*%(B%*%t(B) + lambda*diag(dim(B)[1]))-(Y%*%t(B) + lambda*F%*%U);
  db = (t(Z)%*%Z + lambda2*diag(dim(Z)[2]))%*%B - t(Z)%*%Y;
  du = (t(F)%*%F*lambda + lambda3*diag(dim(U)[1]))%*%U-lambda*t(F)%*%Z;
  
  
  dataout = list(Z = Z, B = B, U = U)
  return(dataout)
}

standardize<- function(X){
  X = as.matrix(X)
  # n = dim(X)[1]
  # p = dim(X)[2]
  
  X = scale(X, center = TRUE, scale = F)
  # X = scale(X,center=FALSE, scale=sqrt(apply(X^2,2,sum)))
  
  # m = apply(X,2,mean)
  # st = sqrt(apply(X^2,2,sum));
  # st_mat = matrix(st, nrow = length(st), ncol = dim(X)[2], byrow=FALSE)
  # X2 = X / st_mat
  return (X)
}

########################################################################
# begin HC estimation or expression PC loading
########################################################################

set.seed(555)

prefix <- paste0(assembly,"_ODS",do_ods,"_TMM",do_tmm,"_VST",do_vst,"_lstpm",lstpm,"_doBC",do_bc)

gene_bed = fread(paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/TCGA_gene_exp_',prefix,'.bed.gz'))
gene_cbt = as.matrix(gene_bed[,7:ncol(gene_bed)])

tx_bed = fread(paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/TCGA_tx_exp_',prefix,'.bed.gz'))
tx_cbt = as.matrix(tx_bed[,7:ncol(tx_bed)])

sum(colnames(gene_cbt)==colnames(tx_cbt))==ncol(gene_cbt) # good
tpm_ids <- colnames(gene_cbt)

# read in prior data
setwd("/rsrch5/home/epi/bhattacharya_lab/data/TCGA/BRCA")

if(method=="HC"){
  dat_rna <- fread("BRCA_txomeMetrics.txt")
  dat_summ <- fread("BRCA_gcSummaryMetrics.txt")
  dat_gen <- fread("BRCA_genomeMetrics.txt")

  sum(dat_rna$sample_ID==dat_summ$sample_ID) # good
  sum(dat_rna$sample_ID==dat_gen$sample_ID) # good

  dat_prior <- cbind(dat_rna, dat_summ[,-1],dat_gen[,-1])
  dat_prior <- data.frame(dat_prior)
  prior_ids <- dat_prior$sample_ID

  dat_prior$prior_ids_sub <- unlist(lapply(strsplit(prior_ids,".rna_seq.transcriptome.gdc_realn.bam"), `[[`, 1))

  # read in hashtag to TCGA id converter
  barcode_dat <- fread('/rsrch5/home/epi/bhattacharya_lab/data/TCGA/BRCA/fastq_hash_TCGA_ID_Full_ID.txt')
  dat_prior <- merge(x=dat_prior,y=barcode_dat,by.x="prior_ids_sub",by.y="V1")
  prior_full_barcodes <- dat_prior$V2

  match_in_prior <- sapply(tpm_ids,FUN=function(x){
    which(prior_full_barcodes==x)
  })
  dat_prior <- dat_prior[match_in_prior,]

  sum(dat_prior$V2==tpm_ids) 

  # remove non-numeric columns
  numeric_cols <- sapply(dat_prior, is.numeric)
  keep <- which(numeric_cols==T)
  dat_prior_sub <-  dat_prior[, keep]

  hcp_gene <- hidden_convariate_linear(standardize(dat_prior_sub), 
                                  standardize(t(gene_cbt)), 
                                  lambda=5,lambda2=1, lambda3=1, 
                                  k=k, iter=100)

  hcp_tx <- hidden_convariate_linear(standardize(dat_prior_sub), 
                                  standardize(t(tx_cbt)), 
                                  lambda=5,lambda2=1, lambda3=1, 
                                  k=k, iter=100)


  }else if(method=="PC"){

  load(paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/pca/TCGA_expr_pca_dat_",prefix,".RData"))
  pc_gene <- data.frame(pca_gene$x)
  pc_tx <- data.frame(pca_tx$x)

  sum(colnames(gene_cbt)==rownames(pc_gene)) # good
  sum(colnames(tx_cbt)==rownames(pc_tx)) # good
  pc_gene <- pc_gene[,1:k]
  pc_tx <- pc_tx[,1:k]

}

## filter out those samples who are repeated and removed

bc_full_tx <- fread(paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/TCGA_tx_exp_",prefix,"_tumor_full_barcodes.txt"),
  header=F)
full_tx <- bc_full_tx$V1[order(bc_full_tx$V1)]
bc_full_gene <- fread(paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/TCGA_gene_exp_",prefix,"_tumor_full_barcodes.txt"),
  header=F)
full_gene <- bc_full_gene$V1[order(bc_full_gene$V1)]

hcp_gene_Z <- data.frame(hcp_gene$Z)
hcp_gene_Z <- hcp_gene_Z[rownames(hcp_gene_Z) %in% full_gene,]

hcp_tx_Z <- data.frame(hcp_tx$Z)
hcp_tx_Z <- hcp_tx_Z[rownames(hcp_tx_Z) %in% full_tx,]

hcp_gene_Z$individualID=substr(rownames(hcp_gene_Z),1,12)
hcp_tx_Z$individualID=substr(rownames(hcp_tx_Z),1,12)

### aggregate covariates

individual <- data.frame(fread("/rsrch5/home/epi/bhattacharya_lab/data/TCGA/BRCA/BRCA_clinical.csv"))

individual$sex_out = factor(individual$gender,levels=c("female","male"))

# individual$stage <- individual$ajcc_pathologic_stage
# individual$stage[individual$stage%in%c("Stage I","Stage IA","Stage IB")] <- "StageI"
# individual$stage[individual$stage%in%c("Stage II","Stage IIA","Stage IIB")] <- "Stage II"
# individual$stage[individual$stage%in%c("Stage III","Stage IIIA","Stage IIIB","Stage IIIC")] <- "Stage III"
# individual$stage[individual$stage=="'--"] <- NA
# individual$stage[individual$stage=="Stage X"] <- NA
#individual$age_at_index_squared <- individual$age_at_index^2

covariates = data.frame(individualID = individual$case_submitter_id,
                        age = individual$age_at_index,
                        sex = individual$sex_out)
#                        stage = individual$stage)

covariates$age[covariates$age=="'--"] <- NA
covariates$age <- as.numeric(covariates$age)
covariates$age[is.na(covariates$age)] = mean(covariates$age,na.rm=T)
covariates$sex[is.na(covariates$sex)] <- "female"
covariates$sex = ifelse(covariates$sex == 'female',0,1)
covariates <- unique(covariates)
rownames(covariates) <- covariates$individualID
#covariates$age2 = covariates$age^2
covariates <- na.omit(covariates) # remove any missingess

covariates_gene <- merge(x=covariates,y=hcp_gene_Z, by="individualID")
covariates_tx <- merge(x=covariates,y=hcp_tx_Z, by="individualID")

colnames(covariates_gene)[-1:-4] = paste0('hcp',1:k)
colnames(covariates_tx)[-1:-4] = paste0('hcp',1:k)

covariates_gene <- na.omit(covariates_gene) # remove any missingess
covariates_tx <- na.omit(covariates_tx) # remove any missingess

print("Dim of gene covariates:")
dim(covariates_gene)
print("Dim of tx covariates:")
dim(covariates_tx)

# restrict samples to only those with good genotypes

pcdat <- fread(gen_pcs_file)
names(pcdat)[1] <- "individualID"
pcdat$individualID <- substr(pcdat$individualID,1,12)
pcdat <- pcdat[,1:(1+num_gen_pcs)]

covariates_gene <- merge(x=covariates_gene,y=pcdat,by="individualID")
#covariates_gene <- covariates_gene[,-which(colnames(covariates_gene)=="stage")]
covariates_tx <- merge(x=covariates_tx,y=pcdat,by="individualID")
#covariates_tx <- covariates_tx[,-which(colnames(covariates_tx)=="stage")]

tcov_tum_gene <- t(covariates_gene)
row.names(tcov_tum_gene)[1] <- "id"

tcov_tum_tx <- t(covariates_tx)
row.names(tcov_tum_tx)[1] <- "id"


########################################################################
# write out files
########################################################################

write.table(tcov_tum_gene,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/covariates/TCGA_cov_qtltools_gene_tumor_",prefix,"_k",k,"_",method,"_genpc",num_gen_pcs,".txt"),
  sep="\t",row.names=T,col.names=F,quote=F)

write.table(tcov_tum_tx,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/covariates/TCGA_cov_qtltools_tx_tumor_",prefix,"_k",k,"_",method,"_genpc",num_gen_pcs,".txt"),
  sep="\t",row.names=T,col.names=F,quote=F)


