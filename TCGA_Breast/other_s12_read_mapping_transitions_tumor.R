...
sampledat <- data.frame(fread("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/sample_lists/sample_list_postQC_final_for_analysis_TCGA_present_all_annot_with_fastq_hash.txt",header=F))

sample_id <- sampledat$V3[sample_ind]
...
process_sample <- function(sample_id) {
  file_A <- file.path(data_dir, paste0(sample_id, "_tumor_gencode.tsv.gz"))   # gencode = Assembly A
  file_B <- file.path(data_dir, paste0(sample_id, "_tumor_LR.tsv.gz"))  # assembly = Assembly B
...
gtf_lr <- data.frame(import("/rsrch5/home/epi/bhattacharya_lab/data/Veiga/STB/SQANTI3/ESPRESSO_tumor_filtered/ESPRESSO_tumor_filtered.gtf"))
...
classif <- data.frame(fread("/rsrch5/home/epi/bhattacharya_lab/data/Veiga/STB/SQANTI3/ESPRESSO_tumor_filtered/ESPRESSO_tumor_filtered_classification.txt"))
...
write.table(flow_counts_all,file=paste0("/rsrch5/scratch/epi/sthead/sankey/flow_counts/tcga_tumor_",gene_abbrev,"_",sample_id,".txt"),
