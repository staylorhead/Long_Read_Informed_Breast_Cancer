...
sampledat <- data.frame(fread("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/GTEx_breast_mammary_bam_files_with_sample_attrib.txt"))
...
process_sample <- function(sample_id) {
  file_A <- file.path(data_dir, paste0(sample_id, "_breast_gencode.tsv.gz"))   # gencode = Assembly A
  file_B <- file.path(data_dir, paste0(sample_id, "_breast_LR.tsv.gz"))  # assembly = Assembly B
...
gtf_lr <- data.frame(import("/rsrch5/home/epi/bhattacharya_lab/data/Veiga/STB/SQANTI3/ESPRESSO_normal_filtered/ESPRESSO_normal_filtered.gtf"))
...
classif <- data.frame(fread("/rsrch5/home/epi/bhattacharya_lab/data/Veiga/STB/SQANTI3/ESPRESSO_normal_filtered/ESPRESSO_normal_filtered_classification.txt"))
...
write.table(flow_counts_all,file=paste0("/rsrch5/scratch/epi/sthead/sankey/flow_counts/gtex_breast_",gene_abbrev,"_",sample_id,".txt"),
