...
keep <- c(keep,grep("gene_gencodev45_k50",file_list))
keep <- c(keep,grep("gene_comb_k50",file_list))
keep <- c(keep,grep("gene_veigaNorm_k100",file_list))
keep <- unique(keep)
...
save(summary_results,file="/rsrch5/home/epi/sthead/isoqtl_lr_breast/results/finemap/GTEx_aggregated_finemap_res_byL_gene.RData")
...
# Combine all results efficiently
summary_results <- rbindlist(summary_results_list, use.names = TRUE, fill = TRUE)
save(summary_results,file="/rsrch5/home/epi/sthead/isoqtl_lr_breast/results/finemap/GTEx_aggregated_finemap_res_byL_tx.RData")
