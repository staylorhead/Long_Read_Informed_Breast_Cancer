...
gtf_file="/rsrch5/home/epi/bhattacharya_lab/data/GTEx_v9/SQANTI3/Cells_Cultured_fibroblasts_filtered/combined_gencodev45_and_ESPRESSO_fibroblast_filtered.gtf"
gtf <- import(gtf_file)
df <- data.frame(
...
load(paste0(finemap_dir,"/GTEx_fibro_aggregated_finemap_res_byL_",level,".RData"))
summary_results$level <- unlist(lapply(strsplit(summary_results$set_name,"_",), `[[`, 3))
summary_results$annot <- unlist(lapply(strsplit(summary_results$set_name,"_",), `[[`, 4))
summary_results$L <- unlist(lapply(strsplit(summary_results$set_name,"_",), `[[`, 11))
...
rsids_high_pip <- unique(rsids_high_pip)
...
out_prefix <- paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/results/gwas_loci/GTEx_fibro_finemap/ld_results_chr",chr,"_",assembly,"_",level,"_k",k,"_L",L)
...
rsids_no_pip <- unique(rsids_no_pip)
