...
keep <- c(keep,grep("gene_gencodev45_k100",file_list))
keep <- c(keep,grep("gene_comb_k100",file_list))
keep <- c(keep,grep("gene_ESPRESSO_k100",file_list))
keep <- unique(keep)
...
save(summary_results,file="/rsrch5/home/epi/sthead/isoqtl_lr_breast/results/finemap/GTEx_fibro_aggregated_finemap_res_byL_gene.RData")
...
#file_list <- file_list[-grep("aggregated",file_list)]
keep <- {}
keep <- c(keep,grep("tx_gencodev45_k50",file_list))
keep <- c(keep,grep("tx_comb_k50",file_list))
keep <- c(keep,grep("tx_ESPRESSO_k50",file_list))
keep <- unique(keep)
file_list <- file_list[keep]
file_list <- file_list[grep("rmcov1",file_list)]
...
# Combine all results efficiently
summary_results <- rbindlist(summary_results_list, use.names = TRUE, fill = TRUE)
save(summary_results,file="/rsrch5/home/epi/sthead/isoqtl_lr_breast/results/finemap/GTEx_fibro_aggregated_finemap_res_byL_tx.RData")
