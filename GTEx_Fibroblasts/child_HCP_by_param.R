...
gene_bed = fread(paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/GTEx_fibroblast_gene_exp_',prefix,'.bed.gz'))
gene_cbt = as.matrix(gene_bed[,7:ncol(gene_bed)])

tx_bed = fread(paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/GTEx_fibroblast_tx_exp_',prefix,'.bed.gz'))
...
# read in prior data
setwd("/rsrch5/scratch/epi/sthead/GTEx_v8/picard_multiqc_with_fibro/multiqc_data")
...
if(method=="HC"){
  ...
  sample_dat <- data.frame(fread("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/GTEx_fibroblast_bam_files_with_sample_attrib.txt"))
  sample_dat$gtex_id <- sub("^([^\\-]+\\-[^\\-]+).*", "\\1", sample_dat$sample)
  rm <- which(sample_dat$SMAFRZE=="EXCLUDE")
  if(length(rm)>0){sample_dat <- sample_dat[-rm,]}
...
write.table(tcov_gene,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/covariates/GTEx_fibro_cov_qtltools_gene_",prefix,"_k",k,"_",method,"_genpc",num_gen_pcs,".txt"),
  sep="\t",row.names=T,col.names=F,quote=F)

write.table(tcov_tx,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/covariates/GTEx_fibro_cov_qtltools_tx_",prefix,"_k",k,"_",method,"_genpc",num_gen_pcs,".txt"),
  sep="\t",row.names=T,col.names=F,quote=F)
