...
process_sample <- function(sample_id) {
  ...
  file_B <- file.path(data_dir, paste0(sample_id, "_fibro_comb.tsv.gz"))  # assembly = Assembly B
...
gtf_lr <- data.frame(import("/rsrch5/home/epi/bhattacharya_lab/data/GTEx_v9/SQANTI3/Cells_Cultured_fibroblasts_filtered/combined_gencodev45_and_ESPRESSO_fibroblast_filtered.gtf"))
...
target_B <- unique(gtf_lr$transcript_id[gtf_lr$gene_id==gene])

transcript_categories$category_B[!transcript_categories$transcript_B %in% target_B] <- paste0("non-",gene_abbrev)
transcript_categories$category_B[transcript_categories$category_B=="full-splice_match"] <- 
...
write.table(flow_counts_all,file=paste0("/rsrch5/scratch/epi/sthead/sankey/flow_counts/gtex_comb_fibro_",gene_abbrev,"_",sample_id,".txt"),
