...
if (assembly == "gencodev45") {
  load("/rsrch5/home/epi/bhattacharya_lab/data/GTEx_v8/tximeta/GTEx_fibroblast_gencodev45.RData")
  gtf="/rsrch5/home/epi/bhattacharya_lab/data/GenomicReferences/txome/gencode_v45/gencode.v45.annotation.gtf"
} else if (assembly == "ESPRESSO") {
  load("/rsrch5/home/epi/bhattacharya_lab/data/GTEx_v8/tximeta/GTEx_fibroblast_ESPRESSO_filtered.RData")
  ...
  load("/rsrch5/home/epi/bhattacharya_lab/data/GTEx_v8/tximeta/GTEx_fibroblast_combined_ESPRESSO_gencodev45.RData")
...
sum(rownames(sample_dat_new)==colnames(se)) # check, 504, good
sample_dat <- sample_dat_new

total_counts <- colSums(assays(se)$counts)
ntx <- nrow(se)
ntx_nonzero <- sum(rowSums(assays(se)$counts)==0) # this should be zero counts, not non zero counts
save(total_counts,ntx, ntx_nonzero,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/tximeta/GTEx_fibroblast_",assembly,"_persample_read_counts.Rdata"))

# remove low quality samples not intended for QTL (already removed)
#rm <- which(sample_dat$SMAFRZE=="EXCLUDE")
#se <- se[,-rm]
#sample_dat <- sample_dat[-rm,]
length(unique(sample_dat$gtex_id))==nrow(sample_dat) # true, n=504
sum(sample_dat$full_gtex_id==colnames(se)) # 504, good

# remove transcripts on chr Y & M & X
if(assembly=="comb"){
  gtf_t <- import(gtf)
  gtf_t <- data.frame(gtf_t)
  gtf_tx <- gtf_t[gtf_t$type == "transcript", ]

  # extract chromosome for each transcript
  tx_chr <- gtf_tx$seqnames[match(names(se), gtf_tx$transcript_id)]

  # filter
  keep <- (!tx_chr %in% c("chrX", "chrY", "chrM"))
  se <- se[keep, ]

  od <- cs$annotation$Overdispersion
  names(od) = rownames(cs$counts)
  save(od,file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/tximeta/GTEx_fibroblast_",assembly,"_OD.Rdata"))
  od <- od[keep]
}else{
  chr <- as.character(seqnames(rowRanges(gr)))
...
# save PCA data
save(pca_tx, pca_gene, file=paste0("/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/pca/GTEx_fibroblast_expr_pca_dat_",prefix,".RData"))

# save tx-level data

if(assembly=="comb"){
  tx_gtf <- data.frame(gtf)
  tx_gtf <- tx_gtf[tx_gtf$type=="transcript",]
  rownames(tx_gtf) <- tx_gtf$transcript_id
  tx_bed <- tx_gtf[rownames(tx_cts),]
  tx_bed <- tx_bed[,c("seqnames","start","end","transcript_id","gene_id","strand")]
...
require(data.table)
fwrite(tx_bed,paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/GTEx_fibroblast_tx_exp_',prefix,'.bed'),
...
if(assembly=="comb"){
  ...
  tx_bed <- tx_gtf[rownames(tx_cts),]
  tx_bed <- tx_bed[,c("seqnames","start","end","transcript_id","gene_id","strand")]
  colnames(tx_bed)<-c("seqnames","start","end","tx_name","gene.id","strand")
  #tx_bed = cbind(tx_bed,tx_cts)
...
fwrite(gene_bed,paste0('/rsrch5/home/epi/sthead/isoqtl_lr_breast/files_for_analysis/bed/GTEx_fibroblast_gene_exp_',prefix,'.bed'),
       sep='\t',col.names=T,row.names=F)
