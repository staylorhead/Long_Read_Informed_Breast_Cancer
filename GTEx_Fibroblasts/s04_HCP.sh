...
# script below will generate files for both gene and tx if they don't already exist
...
if [[ ! -f "${cov_file}" ]]; then
    ...
    Rscript /rsrch5/home/epi/sthead/isoqtl_lr_breast/scripts/GTEx_fibro/child_HCP_by_param.R ${assembly} ${do_ods} ${lstpm} ${do_vst} ${do_tmm} ${do_bc} ${k} ${method} ${gen_pcs_file} ${num_gen_pcs}
else
